...
def setup_working_folder(
    ...
    **gh_opts,
    ...
    if not corpus_version or not metadata_version:
        raise ValueError("cannot proceed since corpus_version and/or metadata_version are undefined")
    ...
    Path(jj(folder, f'corpus_{corpus_version}')).touch()
    Path(jj(folder, f'metadata_{metadata_version}')).touch()
